...
/*
  ...
  bash KissCluster/create_cluster.sh Experiments_N10_W5_SKGpre_OCBA us-east-2 "java -server -Xmx1500M -cp lib/*:bin experiments.Experiments_N10_W5_SKGpre_OCBA" app/ s3://szufel-sim-ohio
 
 */
public class Experiments_N10_W5_SKGpre_OCBA {

	public static void main(String[] args) throws Exception {
		long masterSeed = Long.parseLong(args[0]);
		
		int NUMBER_OF_SIMs = 100;
		int MAX_EXPERIM_BUDGET = 60;
		int W = 5;
		int N  = 10;
		for (int s=0;s<NUMBER_OF_SIMs;s++) {
			...
			ExperimentSimulator skg2 = new ExperimentSimulator(seed, N, null, new ConstDistribution(1), 1, W, new SKG_RSalgorithm(2), 1000);
			skg2.run(MAX_EXPERIM_BUDGET, false);
			
			ExperimentSimulator skg4 = new ExperimentSimulator(seed, N, null, new ConstDistribution(1), 1, W, new SKG_RSalgorithm(4), 1000);
			skg4.run(MAX_EXPERIM_BUDGET, false);
